...
- [ ] M6: Pinned capacity measurement  (DEFERRED 2026-08-27 — blocked on production-model access; not verifiable in this environment, carried forward, see Outcomes)
...
**The recurring pattern worth keeping.** Three milestones independently converged on the same discipline — enforce an invariant by structure, not by a reviewer's vigilance. M3 gave `DriftAssessment` no disposer field, so divergence has nowhere to write a machine decision. M4 made the escape-rate estimator's input a distinct `RandomTranche` type, so a biased sample cannot reach it at a call site. M5 made any logit→disposer assignment a parse-time failure across the live core tree. Each replaces "someone must notice" with "the shape/type/parse makes it impossible", and each was verified adversarially by injecting the exact violation it forbids. This is the same idea the companion patch's Gate-Checkability Audit describes from the compiled-signal side; the retrospective's recommendation is to name it explicitly as a harness principle.

**Technical debt carried forward.** (1) M6, as above. (2) Provisional stand-ins for 9002's unstarted schemas: `QuarantinedFindingGraph`/`EvaluationResult`/`replay_hash`/`derive_evaluation` in `proposer_diagnostics.py`, `assess_drift` in `divergence.py`, and `compute_escape_rate` in `escape_sampler.py` are minimal demonstrators that 9002 replaces — tracked against Q2 and the 9002 cross-plan dependency. (3) The letter-scale token-id mapping is a first-g placeholder pending the config layer (Q3). (4) The dep-vet for `llama-cpp-python` records the downloads check as PASS on the human override plus circumstantial evidence, because pypistats and pepy are both proxy-blocked — a human with unproxied network should confirm >1000/week. (5) The notes-vs-flip-gate contradiction (Q8) was worked around per-milestone; under the promotion rule this is now violation one, and a second occurrence in another plan should move the rule into the test rather than repeating the workaround.

**Steering resolved and outstanding.** Q1 (serving stack) and Q7 (who runs the spike) were resolved by the human's llama.cpp direction and the in-session run. Q2 (FindingGraph on-disk deltas), Q3 (config surface), Q4 (serving shape), Q5 (D-series namespace), Q6 (companion patch 3 scope), and Q8 (notes-vs-gate) remain open and are the honest inheritance for 9002 and the follow-up.

**What I would do differently.** Name the executor environment in the plan's Big Picture, not just the target hardware — the whole M0 stall traced to that gap. And author M0 as two milestones from the start: the capability/capacity split was obvious in hindsight and only became visible after M0 had already been flipped once.

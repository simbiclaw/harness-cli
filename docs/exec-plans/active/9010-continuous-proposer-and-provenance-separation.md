...
### M0 — Logprob capability spike and capacity measurement

Measure, do not assume, two things the rest of the plan is contingent on. First: whether the MLX serving stack surfaces top-k logprobs at a chosen token position, and at what k. Second: actual decode throughput for a hunt pass and a scoring pass on the target box, batched and single-stream, with and without speculative decoding. Both results are captured as a committed experiment artifact under `docs/experiments/9010-logprob-capability/` (a `run.py` plus `output.json`) recording the observed top-k ceiling, the G this plan will therefore use, and measured tokens/second per call type. If no top-k is exposed at all, D19 reduces to argmax scores, only D20 survives, and M2 is rewritten before it starts.

`Acceptance Test:` `tests/test_logprob_capability.py::test_capability_record_is_complete` — the committed artifact parses and declares `topk_available`, `g_used`, and per-call-type throughput; a record missing any of the three fails. `tests/test_logprob_capability.py::test_g_used_within_measured_ceiling` — the recorded `g_used` never exceeds the measured top-k ceiling.
...
- [ ] M0: Logprob capability spike and capacity measurement  (created 2026-08-26)

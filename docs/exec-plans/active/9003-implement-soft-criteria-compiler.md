...
The compiler touches `types/` (node schema, manifest schema, input schemas), `io/` (reading the three compiler inputs, writing `_rubric/` nodes and manifest), `core/` (pure compilation logic — trigger generation, gap classification, escape-tier assignment, hard-fail synthesis), and `cli/` (a `compile` subcommand). It does NOT touch the runtime proposer, grounding gate, or scoring functions — those are 9002's domain.
...
**The authoring procedure** (§2) runs once per soft criterion: A1 (decompose to orthogonal dimensions), A2 (compile signature to falsifiable trigger with `form ∈ {lexical, ordered_relation, threshold, lookup}`), A3 (classify corroborators by independence — redundant class forbidden), A4 (declare the residue — required field), A5 (seed the agreement gate with both tails: κ sample plan + escape sample plan + escape ceiling), A6 (set deduction weight and W_C provisional constant), A7 (register drift detection and iteration policy — re-ground via write-time epoch commit only). Plus A2-ac (author the 12 acoustic indicators into `_rubric/acoustic/`) and A2-ph (author the phrase lexicon into `_rubric/phrase/`).

**The binary→continuous bridge** (§2.6) runs per binary item: B-A (bind item to dimension as weighted evidence via `align.md`, with a `severity_map` reference into the calibration manifest), B-B (compile every NA condition into an `applicability_gate`), B-C (synthesize hard-fail routing rules from item subsets — many-to-one, not copied thresholds), B-D (extract concrete values from checklist text — named phrases → `lexical`, numbers → `threshold`, at confidence 1.0).
...
- `check_no_adjective_triggers(node)` — `trigger.checkable == false` or spec contains evaluative adjectives → reject (AUTH-1)
...
- `check_data_dependency(node)` — `lookup` trigger without `data_dependency`, or `connected: false` without defer disposition → reject (AUTH-8)
...
`Acceptance Test:` `tests/test_validator.py::test_auth1_red` — adjective trigger → validator rejects. `tests/test_validator.py::test_auth1_green` — structural trigger → accepts. One red+green pair per AUTH-1..10, matching the §5 fixtures exactly.

### M2 — Trigger compiler (A1, A2, A2-ac, A2-ph, core)

Land the pure trigger-compilation functions in `src/argus/core/compiler/triggers.py`. The compiler takes a dimension description (from the generic skill template) and a failure signature and produces a `required_evidence` + `fail_condition` block:

- `decompose_dimension(dimension) → list[TriggerCandidate]` — A1: apply independence, exhaustiveness, and operationalizability tests; split what's triggerable from what's residue
- `compile_trigger(candidate) → Trigger` — A2: translate failure signature into `form ∈ {lexical, ordered_relation, threshold, lookup}` with a spec S3 can evaluate deterministically. No adjectives survive. `checkable` must be `true` or the trigger is rejected
- `compile_acoustic_framework(indicators) → list[AuthoredNode]` — A2-ac: author 12 acoustic indicators as rubric nodes, each with threshold and required_evidence shape
- `compile_phrase_lexicon(lexicon) → list[AuthoredNode]` — A2-ph: author phrase/pattern lists as rubric nodes, each with match form

`Acceptance Test:` `tests/test_triggers.py::test_lexical_trigger_compiles` — a phrase-based criterion → `form: "lexical"` trigger. `tests/test_triggers.py::test_ordered_relation_compiles` — "acknowledge before resolve" → `form: "ordered_relation"`, `spec: "ack_span PRECEDES resolution_span WITHIN turn"`. `tests/test_triggers.py::test_adjective_rejected` — "agent should sound empathetic" → rejected, `checkable: false`. `tests/test_triggers.py::test_acoustic_framework_has_12_indicators` — A2-ac output has exactly 12 nodes. `tests/test_triggers.py::test_phrase_lexicon_output` — A2-ph output has sensitive/negative/recommended/asr_bias sections.
...
- `declare_residue(trigger, dimension) → str` — A4: name what the trigger does NOT capture. Required field; empty → AUTH-2 rejects.
- `classify_gap(item, dimension, trigger) → GapClassification` — rev.4: classify the gap as values, perceiver, proxy, calibration_surface_form, or coverage. Determines `escape_tier` (standard vs aggressive) and whether AUTH-9 auto-final ban applies.
...
- `extract_values(item) → list[ValueExtraction]` — B-D: parse named phrases → `lexical` spec, numeric thresholds → `threshold` spec, at `confidence 1.0`. Never settle for principle-level when the rubric text carries the exact value.
...
`Acceptance Test:` `tests/test_manifest_channel.py::test_manifest_injection_reanchors_severity` — nodes' severity_map refs update to new epoch, triggers unchanged. `tests/test_manifest_channel.py::test_auth9_reevaluated_on_injection` — a surface-form node that was `auto_final: false` gets `auto_final: true` after manifest injection covers its failure surface. `tests/test_manifest_channel.py::test_manifest_injection_no_recompile` — injection does not change triggers, corroborators, or agreement blocks. `tests/test_manifest_channel.py::test_manifest_epoch_independent_of_rule_epoch` — manifest epochs advance independently; rule epochs unchanged by manifest injection.

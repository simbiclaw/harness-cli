...
`Allowed Reads: docs/retrospectives/soft-criteria-authoring-spec-v4.html, docs/retrospectives/soft-criteria-authoring-spec-v4-patch-1.md, docs/retrospectives/soft-criteria-authoring-spec-v4-patch-2.md, src/argus/types/**, src/argus/core/compiler/validator.py, src/argus/core/compiler/signals.py, src/argus/core/compiler/classify.py, docs/conventions/layering.md`
`Allowed Writes: src/argus/core/compiler/agreement.py, tests/test_agreement_seed.py, docs/exec-plans/active/9003-implement-soft-criteria-compiler-notes/`
`Requires: M3`
`Risk Tier: B`
...
### M1 adversarial verification (2026-08-12)
...
### M4 adversarial verification (2026-08-12)
...
* 2026-08-13 (INTENTS writer model — canonical sources): the tree's four-way filling model is documented in the tree's OWN metadata — `INTENTS/MANIFEST.yaml` ("L1 curated · L2 doc2graph · L3 audio2tree"; per-node `source:` annotations) and `INTENTS/AGENTS.md` (transformation-layer writers: audio2tree, doc2graph, criteria-compiler, navigator; "不直接写"), plus `docs/retrospectives/audio2tree-intents-9003-relationship.md` (producer-ownership tables). The 9003 compiler IS one of the writers (AGENTS.md writer list) — its shelf is `_rubric/`, consistent with this plan's output contract; L2/L3 case nodes are audio2tree's (bottom-up) and doc2graph's (top-down) domain, not the compiler's. **Lesson: the INTENTS tree's own metadata is the authoritative source for producer ownership — read MANIFEST.yaml/AGENTS.md before reasoning about tree filling; a prior exploration listed MANIFEST.yaml without reading it, leading to a mistaken 'compiled rubric lands in L2/L3' premise (corrected 2026-08-13).**
* 2026-08-13 (process lesson): the M8 gating inputs — the real Specific QA Rubric (`docs/PRD/eval/rubric_com_hotline.md`) and real `align.md` (`docs/PRD/eval/align.md`) — existed in the repo since BEFORE plan creation, yet the spec and this plan declared them "PENDING INPUTS — hand to Claude Code" for the entire M0–M7 build without verifying existence. A one-command check (`ls docs/PRD/eval/`) at plan creation would have found both. The fixture's item-20/21 examples were themselves derived from the real rubric, so the real inputs were indirectly in the loop all along. Impact: ZERO rework of M0–M7 (infrastructure is input-agnostic by design); M8's actual work — input conversion (27 items → compiler schemas) + the worked compilation over the 25 operational items — was always pending and is now un-gated. The real align.md's per-item compilability annotations (lexical/lookup/threshold/ordered_relation/acoustic + confidence + residue) also reveal signal forms the M2 core does not yet emit (lookup, acoustic prosody) — genuine M8 discovery work, not rework. **Prevention (documentation-level rule): milestones gated on external inputs must verify input existence in-repo at plan creation; promotion candidate if this recurs across ExecPlans.**

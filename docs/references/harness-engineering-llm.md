...
The initial scaffold—repository structure, CI configuration, formatting rules, package manager setup, and application framework—was generated by Codex CLI using GPT‑5, guided by a small set of existing templates. Even the initial AGENTS.md (equivlent of CLAUDE.md) file that directs agents how to work in the repository was itself written by Codex.
...
We tried the “one big [`AGENTS.md`(equivlent of CLAUDE.md)](https://agents.md/)” approach. It failed in predictable ways:
...
So instead of treating `AGENTS.md` (equivlent of CLAUDE.md) as the encyclopedia, we treat it as **the table of contents**.

The repository’s knowledge base lives in a structured `docs/` directory treated as the system of record. A short `AGENTS.md`  is injected into context and serves primarily as a map, with pointers to deeper sources of truth elsewhere.
